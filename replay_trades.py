...
def main():
    ...
    use_htf = htf_cfg.get("use", True)
    if args.use_htf_filter is not None:
        use_htf = args.use_htf_filter
    htf_timeframe = args.htf_timeframe or htf_cfg.get("timeframe", "4h")
    htf_period = htf_cfg.get("supertrend_period", 14)
    htf_multiplier = htf_cfg.get("supertrend_multiplier", 2.5)
    ...
    trend_filter = None
    if use_htf:
        print(f"Загрузка HTF: {symbol} {htf_timeframe}")
        df_htf = fetch_ohlcv_all(ex, symbol, timeframe=htf_timeframe, max_bars=args.max_bars)
        _, htf_trend_up = calculate_supertrend(df_htf, htf_period, htf_multiplier)
